...
> **Reihenfolge-Hinweis (Codex-Gate):** Der **Produktiv-Upgrade (UP-1) gehört VOR**
> produktive Config-/UX-Auslieferung. Lokal ist die Rehearsal erledigt; auf Live wird
> erst 5.1.0 aufgespielt, dann die Quick-Wins aktiviert. Auf `zhl-main` dürfen QW/UX
> parallel **vorbereitet** werden (Deployment-Reihenfolge ≠ Entwicklungsreihenfolge).
> **Betrieb (SFTP-only):** Cron-Jobs (`sendreminders.php`, `sendwaitlist.php`,
> `sendmissedcheckin.php`) laufen im Container — **offen: wer steuert die Cron-Ausführung?**
> (siehe PROGRESS „Offene Entscheidungen").

## Produktiv-Upgrade zuerst (Säule 1)
| WP | Inhalt | Aufwand |
|---|---|---|
| UP-1 | Wartungsfenster, finales Backup, 5.1.0-Deploy per SFTP, DB-Stempel, Smoke-Test, Cron prüfen | M |

## Dann — Config-Quick-Wins (kein/kaum Code, je 1 PR)
...
| QW-4 | Waitlist (+ Cron `sendwaitlist`) einschalten — **kein** F34-Ersatz | F36 | S |
| QW-5 | DE-Mailtemplates (`lang/de_de/<Event>-custom.tpl`) | F22 | S |
| QW-6 | iCal-Subscription / API gezielt freigeben (Privacy prüfen!) | F20, F28 | XS |
...
| CM-3 | Accessory-Erweiterung: Zustand + Seriennummer (**vor/mit CM-2**) | F30 | S |
| CM-2 | **QR-Verifikations-Checkliste** im Check-in/out (nutzt CM-3-Accessories) | F10, F30 | M |
...
1. **CM-4 Stufe 1** (F40 Gruppen-Gate ohne Code an der lokalen Kopie durchspielen +
   als **Config-Runbook** dokumentieren — kein Custom-PR).
2. **QW-2…QW-6** als ein „Config-Quick-Wins"-PR auf `zhl-main` vorbereiten
   (Aktivierung auf Live erst nach UP-1).
3. **UX-1 + UX-2** (Landing verdrahten + ZHL-Branding via `css.extension.file`).
4. **UP-1** mit dir terminieren (Wartungsfenster) → dann Quick-Wins live.
