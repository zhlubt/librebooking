...
-- IF NOT EXISTS → idempotent (MariaDB), wiederholtes Ausführen bricht nicht.
ALTER TABLE zhl_handover_check_item
  ADD COLUMN IF NOT EXISTS label VARCHAR(255) NULL AFTER accessory_id;

-- Gesamtzustand des Geräts bei der Übergabe (F30).
ALTER TABLE zhl_handover_check
  ADD COLUMN IF NOT EXISTS overall_condition ENUM('ok','minor','damaged') NULL AFTER type;
