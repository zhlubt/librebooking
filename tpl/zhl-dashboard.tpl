...
<div class="zhl-dash">
	...
	<div class="zhl-dash-head">
		...
		<div class="d-flex gap-2 align-items-center">
			...
			{if $IsAdmin}<a class="zhl-btn zhl-btn-sm" href="{$Path}zhl-bundles-admin.php">⚙ <span data-en="Bundles">Bundles</span></a><a class="zhl-btn zhl-btn-sm" href="{$Path}zhl-certificates-admin.php">🎓 <span data-en="Certificates">Zertifikate</span></a>{/if}
		</div>
	</div>

	<nav class="zhl-modenav">
		<a class="zhl-mode" href="{$Path}zhl-assistant.php">🎯 <span data-en="Book bundles">Bundles buchen</span></a>
		<a class="zhl-mode active" href="{$Path}zhl-dashboard.php">🎛 <span data-en="Book individual devices">Geräte einzeln buchen</span></a>
		<a class="zhl-mode" href="{$Path}zhl-bookings.php">📅 <span data-en="My Bookings">Meine Buchungen</span></a>
				...
				{foreach from=$Rows item=row}
					<div class="zhl-card {if !$row->anyFree}is-full{/if}">
						...
						{if $row->minNoticeDays > 0}
							<div class="zhl-vorlauf-note">⏳ <span data-en="Lead time">Vorlauf</span> {$row->minNoticeDays} <span data-en="days">Tage</span> — <span data-en="earliest start">frühester Start</span>: <strong>{$row->earliestLabel}</strong></div>
						{/if}
