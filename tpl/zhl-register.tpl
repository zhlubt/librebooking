...
<title>Konto anlegen – Medienausleihe ZHL</title>
{literal}
<style>
  :root {
  ...
  * { box-sizing:border-box; }
  ...
  .btn-login { width:100%; border:none; cursor:pointer; background:var(--grad); color:#fff; font-weight:600; font-size:16px; padding:13px; border-radius:var(--r-sm); box-shadow:0 5px 14px rgba(0,146,96,.28); display:inline-flex; align-items:center; justify-content:center; gap:9px; font-family:inherit; transition:transform .12s var(--ease),box-shadow .12s var(--ease); margin-top:6px; }
  .btn-login:hover { transform:translateY(-1px); box-shadow:0 9px 20px rgba(0,146,96,.36); }
  .btn-login svg { width:18px; height:18px; }
  .register-hint { text-align:center; font-size:14.5px; color:var(--muted); margin-top:20px; }
...
<main>
  <div class="wrap">
    <form role="form" name="register" id="register" method="post" action="{$smarty.server.SCRIPT_NAME}?action=register">
    {csrf_token}
    <div class="login-card">

      <div class="explain">
        <p class="eyebrow">Medienausleihe · ZHL</p>
        <h1 data-en="Create your lending account">Konto für die Ausleihe anlegen</h1>
        <p data-en="A separate account for the media lending service — quick and with as little data as possible.">Ein eigenes Konto für die Medienausleihe — schnell und mit so wenig Daten wie möglich.</p>
        <div class="points">
          <div class="point">
            ...
            <div><b data-en="University email only">Nur Uni-Mailadresse</b><span data-en="Registration works only with an @uni-bayreuth.de or @myubt.de address.">Registrierung nur mit einer @uni-bayreuth.de- oder @myubt.de-Adresse.</span></div>
          </div>
          <div class="point">
            <span class="pic"><svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg></span>
            <div><b data-en="Confirm by email">Bestätigung per E-Mail</b><span data-en="After registering you'll get an email with an activation link.">Nach der Registrierung erhalten Sie eine E-Mail mit Bestätigungs-Link.</span></div>
          </div>
          <div class="point">
            ...
            <div><b data-en="Your own password">Eigenes Passwort</b><span data-en="You set a separate password here, not the central university password.">Sie legen sich hier ein eigenes Passwort an, nicht das zentrale Uni-Passwort.</span></div>
          </div>
        </div>
      </div>

      <div class="form">
        <h2 data-en="Register">Registrieren</h2>
        <p class="sub" data-en="Just four details — that's all we need.">Nur vier Angaben — mehr brauchen wir nicht.</p>
        ...
        <div class="field">
          <label for="email" data-en="University email address">Uni-Mailadresse</label>
          <div class="input-wrap">
            ...
            <input id="email" type="email" required {formname key=EMAIL} value="{$Email|escape}"
                   pattern=".+@(uni-bayreuth\.de|myubt\.de)$"
                   title="Bitte eine @uni-bayreuth.de- oder @myubt.de-Adresse verwenden."
                   placeholder="vorname.nachname@uni-bayreuth.de" autocomplete="email">
          </div>
          <p class="hint" data-en="Also serves as your username.">Dient zugleich als Ihr Benutzername.</p>
        </div>

        <div class="grid2">
          <div class="field">
            <label for="fname" data-en="First name">Vorname</label>
            <div class="input-wrap">
              ...
              <input id="fname" type="text" required {formname key=FIRST_NAME} value="{$FirstName|escape}" autocomplete="given-name">
            </div>
          </div>
          <div class="field">
            <label for="lname" data-en="Last name">Nachname</label>
            <div class="input-wrap">
              <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
              <input id="lname" type="text" required {formname key=LAST_NAME} value="{$LastName|escape}" autocomplete="family-name">
            </div>
          </div>
        </div>
        ...
        <div class="field">
          ...
          <div class="input-wrap">
            ...
            <input id="password" type="password" required minlength="8" {formname key=PASSWORD}
                   placeholder="mindestens 8 Zeichen" data-en-ph="at least 8 characters" autocomplete="new-password">
          </div>
          <p class="hint" data-en="At least 8 characters. This is your own lending password.">Mindestens 8 Zeichen. Das ist Ihr eigenes Ausleih-Passwort.</p>
        </div>

        {* Captcha ist deaktiviert (E-Mail-Bestätigung übernimmt den Schutz) — Feld bleibt als No-Op, falls
           wieder aktiviert wird, validiert der NullCaptchaService leer durch. *}
        ...
        <button class="btn-login" type="submit">
          ...
          <span data-en="Create account">Konto anlegen</span>
        </button>

        <p class="register-hint">
          <span data-en="Already have an account?">Schon ein Konto?</span>
          <a href="index.php"><span data-en="Sign in">Anmelden</span></a>
        </p>
      </div>
...
{literal}
<script>
  var TITLE = { de: "Konto anlegen – Medienausleihe ZHL", en: "Create account – Media Lending ZHL" };
  var cur = "de";
